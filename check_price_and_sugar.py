...
def check_price_and_sugar(tbl):
    ...
    cmap = cm.plasma
    norm = matplotlib.colors.Normalize(vmin=tbl['winpercent'].min(), vmax=tbl['winpercent'].max(), clip=True)
    
    # Plot the data points
    for i in range(len(tbl)):
        draw_a_candy(tbl['sugarpercent'][i], tbl['winpercent'][i], cmap(norm(tbl['winpercent'][i])), s=tbl['winpercent'][i])
